	...
	if(isset($_REQUEST['club_id']) && isset($_REQUEST['action'])) {
		...
		if($_REQUEST['action'] == "view") {
			...
			if($applicationId !== FALSE) {
				...
				get_page("message", array("stylepath" => "../style", "title" => "Internal error", "message" => "Internal error: application not found."));
			}
			...
			get_page("message", array("stylepath" => "../style", "title" => "Saved", "message" => "Your application has been saved. Click <a href\"app.php?$clubGet&action=view\">here</a> to continue."));
		}
	} else {
		get_page("message", array("stylepath" => "../style", "title" => "Internal error", "message" => "Internal error: club ID or action not specified."));
	}
} else {
