...
if(isset($_SESSION['admin_id'])) {
	...
	if($club_id != 0) {
		if(isset($_REQUEST['description']) && isset($_REQUEST['view_time']) && isset($_REQUEST['open_time']) && isset($_REQUEST['close_time'])) {
			...
			mysql_query("UPDATE clubs SET description='$description', view_time='$view_time', open_time='$open_time', close_time='$close_time' WHERE id='$club_id'");
		}
		
		$result = mysql_query("SELECT description, view_time, open_time, close_time FROM clubs WHERE id='$club_id'");
